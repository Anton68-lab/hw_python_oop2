...
class SportsWalking(Training):
    ...
    COEF_SPENT_CAL: float = 0.035
    COEF_SPENT_CALORIES: float = 0.029
    EXPONENT: int = 2
    CM_IN_METR: int = 100
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        return ((self.COEF_SPENT_CAL * self.weight
                 + ((((self.get_mean_speed() * self.KMH_IN_MSEC)
                      ** self.EXPONENT) / (self.height / self.CM_IN_METR))
                    * (self.COEF_SPENT_CALORIES * self.weight)))
                * (self.duration * self.MIN_IN_HOUR))
